...
cWeaponTest::cWeaponTest(char* szFolder, char* szFilename)
	: m_pRootFrame(NULL)
	...
	, m_vPosition(0, 0, 0)
{
	cWeaponTest* pSkinnedMesh = (cWeaponTest*)g_pSkinnedMeshManager->GetSkinnedMesh(szFolder, szFilename);
...
cWeaponTest::~cWeaponTest(void)
{
	//Destroy();
	SAFE_RELEASE(m_pAnimController);
}

void cWeaponTest::Load(char* szDirectory, char* szFilename)
{
...
void cWeaponTest::UpdateAndRender()
{
	...
	if (m_pRootFrame)
	{
		...
		Update(m_pRootFrame, (D3DXMATRIXA16*)&m_matRWeaponTM);
		Render(m_pRootFrame);
	}
}

void cWeaponTest::UpdateAndRender(D3DXMATRIX * pCameraMat)
{
	if (m_pAnimController)
	{
		m_pAnimController->AdvanceTime(g_pTimeManager->GetDeltaTime(), NULL);
	...
	{
		D3DXMATRIXA16 mat;
		mat = m_matRWeaponTM * (D3DXMATRIXA16)*pCameraMat;
		//D3DXMatrixTranslation(&mat, m_vPosition.x, m_vPosition.y, m_vPosition.z);
...
void cWeaponTest::Render(ST_BONE* pBone /*= NULL*/)
{
	assert(pBone);
...
LPD3DXEFFECT cWeaponTest::LoadEffect(char* szFilename)
{
...
void cWeaponTest::Update(ST_BONE* pCurrent, D3DXMATRIXA16* pmatParent)
{
...
void cWeaponTest::SetupBoneMatrixPtrs(ST_BONE* pBone)
{
	assert(pBone);
...
void cWeaponTest::SetAnimationIndex(int nIndex)
{
	if (!m_pAnimController)
		return;
...
void cWeaponTest::SetRandomTrackPosition()
{
