...
cFace::cFace(char* szFolder, char* szFilename)
	: m_pRootFrame(NULL)
	...
	, m_vPosition(0, 0, 0)
{
	cFace* pSkinnedMesh = (cFace*)g_pSkinnedMeshManager->GetSkinnedMesh(szFolder, szFilename);
...
void cFace::Load(char* szDirectory, char* szFilename)
{
...
void cFace::UpdateAndRender()
{
	...
	if (m_pRootFrame)
	{
		...
		Update(m_pRootFrame, (D3DXMATRIXA16*)&m_matNeckTM);
		Render(m_pRootFrame);
	}
}

void cFace::UpdateAndRender(D3DXMATRIX * pCameraMat)
{
	if (m_pAnimController)
	{
		m_pAnimController->AdvanceTime(g_pTimeManager->GetDeltaTime(), NULL);
	...
	{
		D3DXMATRIXA16 mat;
		mat = m_matNeckTM * (D3DXMATRIXA16)*pCameraMat;
		//D3DXMatrixTranslation(&mat, m_vPosition.x, m_vPosition.y, m_vPosition.z);
...
void cFace::Render(ST_BONE* pBone /*= NULL*/)
{
	assert(pBone);
...
LPD3DXEFFECT cFace::LoadEffect(char* szFilename)
{
...
void cFace::Update(ST_BONE* pCurrent, D3DXMATRIXA16* pmatParent)
{
...
void cFace::SetupBoneMatrixPtrs(ST_BONE* pBone)
{
	assert(pBone);
...
void cFace::SetRandomTrackPosition()
{
