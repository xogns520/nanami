...
cHair::cHair(char* szFolder, char* szFilename)
	: m_pRootFrame(NULL)
...
void cHair::Load(char* szDirectory, char* szFilename)
{
...
void cHair::UpdateAndRender()
{
	...
	if (m_pRootFrame)
	{
		...
		Update(m_pRootFrame, (D3DXMATRIXA16*)&m_matHairTM);
		Render(m_pRootFrame);
	}
}

void cHair::Render(ST_BONE* pBone /*= NULL*/)
{
	assert(pBone);
...
LPD3DXEFFECT cHair::LoadEffect(char* szFilename)
{
...
void cHair::Update(ST_BONE* pCurrent, D3DXMATRIXA16* pmatParent)
{
...
void cHair::SetupBoneMatrixPtrs(ST_BONE* pBone)
{
	assert(pBone);
...
void cHair::SetRandomTrackPosition()
{
