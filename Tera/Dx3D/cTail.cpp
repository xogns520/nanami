...
cTail::cTail(char* szFolder, char* szFilename)
	: m_pRootFrame(NULL)
...
{
	cTail* pSkinnedMesh = (cTail*)g_pSkinnedMeshManager->GetSkinnedMesh(szFolder, szFilename);
...
void cTail::Load(char* szDirectory, char* szFilename)
{
...
void cTail::UpdateAndRender()
{
	...
	if (m_pRootFrame)
	{
		...
		Update(m_pRootFrame, (D3DXMATRIXA16*)&m_matTailTM);
		Render(m_pRootFrame);
	}
}

void cTail::Render(ST_BONE* pBone /*= NULL*/)
{
	assert(pBone);
...
LPD3DXEFFECT cTail::LoadEffect(char* szFilename)
{
...
void cTail::Update(ST_BONE* pCurrent, D3DXMATRIXA16* pmatParent)
{
...
void cTail::SetupBoneMatrixPtrs(ST_BONE* pBone)
{
	assert(pBone);
...
void cTail::SetRandomTrackPosition()
{
